...
elFinder.prototype.quickLook = function(fm, el) {
	...
	this.mimes = {};
self.fm.log(navigator.mimeTypes.length)
	...
	self.fm.log(navigator.mimeTypes)
	// self.fm.log(this.mimes)
	// self.fm.log(navigator.plugins)
	/**
	...
	function update() {
		...
		for (var i in self.plugins) {
			if (self.plugins[i].test && self.plugins[i].test(f.mime, self.mimes)) {
...
elFinder.prototype.quickLook.prototype.plugins = {
	...
	image : new function() {
		...
		this.test = function(mime) {
			return mime.match(/image\/(jpeg|png|gif)/);
		}
		...
		this.show = function(ql, f) {
			...
			if (url && f.hash == ql._hash) {
				$('<img/>').hide().appendTo(ql.media.show()).attr('src', url).load(function() {
					t = $(this).unbind('load');
					if (f.hash == ql._hash) { 
