...
abstract class elFinderVolumeDriver {
	...
	public function mount(array $opts) {
		...
		$this->nameValidator = !empty($this->options['acceptedName']) ? $this->options['acceptedName']: '';
	...
	protected function nameAccepted($name) {
		if ($this->nameValidator) {
			if (is_string($this->nameValidator)	&& function_exists($this->nameValidator)) {
				$f = $this->nameValidator;
				return $f($name);
			} elseif (is_array($this->nameValidator)
			&& count($this->nameValidator) > 1
			&& is_object($this->nameValidator[0])
			&& method_exists($this->nameValidator[0], $this->nameValidator[1])) {
				$obj    = $this->nameValidator[0];
				$method = $this->nameValidator[1];
