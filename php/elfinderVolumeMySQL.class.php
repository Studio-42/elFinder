...
class elFinderVolumeMySQL extends elFinderVolumeDriver {
	...
	protected function init() {
		$this->locale = isset($this->options['locale']) ? $this->options['locale'] : '';
		if (!($this->options['host'] || $this->options['socket'])
		||  !$this->options['user']
		||  !isset($this->options['pass'])
		||  !$this->options['db']
		||  !$this->options['path']
	 ...
	 **/
	protected function query($sql, $debug = false) {
		$this->sqlCnt++;
		...
		if ($debug) {
			...
			$index = 2;
			$sql = 'Called in '.basename($caller[$index]['file']).', line '.$caller[$index]['line']."\n$sql\n";
			\Log::info($sql);
		}
		return $res;
	}
	...
	/**
	 * Ajoute l'enregistrement dans le résultat pour l'affichage
	 *
	 * @param $row
	 * @author dvpweb
	 */
	protected function _addRowForCacheDir($path, $row) {
		// debug($row);
		$id = $row['id'];
		if ($row['parent_id']) {
	 ...
	 * @return string
	 * @author Dmitry Levashov, dvpweb
	 **/
	protected function cacheDir($path) {
		$this->dirsCache[$path] = array();
		if ($this->tbf == 'elfinder_file_recycled') {
			$sql = "SELECT f.id, f.parent_id, f.name, f.size, f.mtime AS ts, f.mime, f.read, f.write, f.locked, f.hidden, f.width, f.height, 
						f.delete_user_id, f.deleted_at, IF(ch.id, 1, 0) AS dirs 
					FROM elfinder_file AS f 
					LEFT JOIN elfinder_file AS ch ON ch.parent_id=f.id AND ch.mime='directory'
					WHERE f.id>0 AND f.parent_id>='".($path == 2 ? 1 : $path)."' AND f.deleted_at IS NOT NULL GROUP BY f.id ";
			$res = $this->query($sql);
			if ($res) {
				while ($row = $res->fetch_assoc()) {
					$this->_addRowForCacheDir($path, $row);
					// Si le parent de l'enfant n'est supprimé alors qu'on souhaiterait l'afficher dans la corbeille,
					// alors on le rajoute au résultat, récursivement jusqu'à trouver le dernier
					$curParent = $row['parent_id'];
					if ($curParent) {
						if ($this->tbf == 'elfinder_file_recycled') {
							$i = 0;
							while ($curParent > $path) {
								$i++;
								if ($i>10) {
									throw new Exception(trans('dialog.error_too_deep', [], 'messages', $this->locale));
									break;
								}
								$sql2 = "SELECT f.id, f.parent_id, f.name, f.size, f.mtime AS ts, f.mime, f.read, f.write, f.locked, f.hidden, f.width, f.height, 
										f.delete_user_id, f.deleted_at FROM elfinder_file AS f 
										WHERE f.deleted_at IS NULL AND f.id = {$curParent}";
								$res2 = $this->query($sql2);
								$curParent = 0;
								if ($res2) {
									while ($row2 = $res2->fetch_assoc()) {
										$curParent = $row2['parent_id'];
										if ($curParent == $path) {
											$this->_addRowForCacheDir($path, $row2);
										}
									}
								}
								unset($res2);
			...
			$sql = "SELECT f.id, f.parent_id, f.name, f.size, f.mtime AS ts, f.mime, f.read, f.write, f.locked, f.hidden, f.width, f.height, 
					...
					FROM elfinder_file AS f 
					LEFT JOIN elfinder_file AS ch ON ch.parent_id=f.id AND ch.mime='directory'
					WHERE f.id>0 AND f.parent_id='{$path}' AND (f.deleted_at IS NULL OR (f.mime='directory' AND f.id IN (
					SELECT parent_id FROM elfinder_file 
						WHERE parent_id IN (
							SELECT parent_id FROM elfinder_file WHERE mime!='directory' AND deleted_at IS NULL GROUP BY parent_id
						) AND deleted_at IS NOT NULL GROUP BY parent_id)
					)) GROUP BY f.id ";
			$res = $this->query($sql);
			if ($res) {
				while ($row = $res->fetch_assoc()) {
	...
	protected function doSearch($path, $q, $mimes) {
		...
		$sql = "SELECT f.id, f.parent_id, f.name, f.size, f.mtime AS ts, f.mime, f.read, f.write, f.locked, f.hidden, f.width, f.height, f.delete_user_id, f.deleted_at, 0 AS dirs 
				FROM %s AS f 
				WHERE %s";
		...
		if (($res = $this->query($sql))) {
			while ($row = $res->fetch_assoc()) {
				...
				if ($row['parent_id']) {
					...
					if ($this->tbf=='elfinder_file_recycled') $parent_id = $parent_id==2 ? 1 : $parent_id;
					$row['phash'] = $this->encode($parent_id);
				}
	...
	protected function _basename($path) {
		if ($stat = $this->stat($path)) {
			return ($this->locale && strpos($stat['name'], '.') !== false ? trans($stat['name'], [], 'messages', $this->locale) : $stat['name']);
		}
		return false;
	}
	...
	protected function _path($path) {
		...
		foreach ($parentsIds as $id) {
			...
			if (strpos($dir['name'], '.') !== false) {
				$dir['name'] = trans($dir['name'], [], 'messages', $this->locale);
			}
	...
	/**
	 * Modfie l'enregistrement dans le résultat en paramètrre
	 *
	 * @param $stat
	 * @author dvpweb
	 */
	protected function _addRowForStat(&$stat) {
        if ($stat['id']==2 && strpos($stat['name'], '.') !== false) {
            $stat['name'] = trans($stat['name'], [], 'messages', $this->locale);
        }
       if ($stat['parent_id']) {
	...
	protected function _stat($path) {
		if ($this->tbf == 'elfinder_file_recycled') {
			$sql = "SELECT f.id, f.parent_id, f.name, f.size, f.mtime AS ts, f.mime, f.read, f.write, f.locked, f.hidden, f.width, f.height, f.delete_user_id, f.deleted_at, IF(ch.id, 1, 0) AS dirs
					FROM elfinder_file AS f 
					LEFT JOIN elfinder_file AS p ON p.id=f.parent_id
					LEFT JOIN elfinder_file AS ch ON ch.parent_id=f.id AND ch.mime='directory'
					WHERE f.id='".($path == 2 ? 1 : $path)."' GROUP BY f.id ";
			$res = $this->query($sql);
			if ($res) {
				$stat = $res->fetch_assoc();
				$this->_addRowForStat($stat);
				return $stat;
			}
		} else {
			$sql = "SELECT f.id, f.parent_id, f.name, f.size, f.mtime AS ts, f.mime, f.read, f.write, f.locked, f.hidden, f.width, f.height, f.delete_user_id, f.deleted_at, IF(ch.id, 1, 0) AS dirs
					...
					LEFT JOIN elfinder_file AS p ON p.id=f.parent_id
					LEFT JOIN elfinder_file AS ch ON ch.parent_id=f.id AND ch.mime='directory'
					WHERE f.id='{$path}' AND (f.deleted_at IS NULL OR (f.mime='directory' AND f.id IN (
					SELECT parent_id FROM elfinder_file 
						WHERE parent_id IN (
							SELECT parent_id FROM elfinder_file WHERE mime!='directory' AND deleted_at IS NULL GROUP BY parent_id
						) AND deleted_at IS NOT NULL GROUP BY parent_id)
	...
	protected function _copy($source, $targetDir, $name) {
		...
		$sql = $id > 0
			? sprintf("REPLACE INTO %s (id, parent_id, name, content, size, mtime, mime, width, height, `read`, `write`, `locked`, `hidden`) ".
						"(SELECT %d, %d, name, content, size, mtime, mime, width, height, `read`, `write`, `locked`, `hidden` FROM %s WHERE id=%d)",
						$this->tbf, $id, $this->_dirname($id), $this->tbf, $source)
			: sprintf("INSERT INTO %s (parent_id, name, content, size, mtime, mime, width, height, `read`, `write`, `locked`, `hidden`) ".
						"SELECT %d, '%s', content, size, %d, mime, width, height, `read`, `write`, `locked`, `hidden` FROM %s WHERE id=%d",
						$this->tbf, $targetDir, $this->db->real_escape_string($name), time(), $this->tbf, $source);
	...
	protected function _unlink($path) {
		...
		$sql = sprintf("UPDATE %s SET deleted_at=%s, delete_user_id=%s WHERE id=%d AND mime!='directory' LIMIT 1",
			$this->tbf, DB::raw('UTC_TIMESTAMP'), $user_id, $path);
	 ...
	 * @author dvpweb
	 **/
	protected function _undoUnlink($path) {
		$user_id = @\Session()->user()->id;
		$user_id = $user_id>0 ? $user_id : 'NULL';
		$sql = sprintf("UPDATE %s SET deleted_at=NULL, updated_at=%s, update_user_id=%s WHERE id=%d AND mime!='directory' LIMIT 1",
			$this->tbf, DB::raw('UTC_TIMESTAMP'), $user_id, $path);
		return $this->query($sql) && $this->db->affected_rows;
	}
	 ...
	 * @author dvpweb
	 **/
	protected function _undoRmdir($path) {
		$user_id = @\Session()->user()->id;
		$user_id = $user_id>0 ? $user_id : 'NULL';
		$sql = sprintf("UPDATE %s SET deleted_at=NULL, updated_at=%s, update_user_id=%s WHERE id=%d AND mime='directory' LIMIT 1",
			$this->tbf, DB::raw('UTC_TIMESTAMP'), $user_id, $path);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function _orig_unlink($path) {
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function _orig_rmdir($path) {
		return $this->query(sprintf("DELETE FROM %s WHERE id=%d AND mime='directory' LIMIT 1", $this->tbf, $path)) && $this->db->affected_rows;
	}
